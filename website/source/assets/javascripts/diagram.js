...
var lyra = employee.clone().position(300, 50).attr('text/text', 'Automation Service \n(lyra)');
var arcApi = employee.clone().position(300, 200).attr('text/text', 'Arc API');
var dashboard = number.clone().position(50, 50).attr('text/text', 'Dashboard');
var cli = number.clone().position(50, 200).attr('text/text', 'Cli');
...
graph.addCells([dashboard, cli, lyra, arcApi]);

dashToLyra = createLink(dashboard, lyra);
dashToArcApi = createLink(dashboard, arcApi);
cliToLyra = createLink(cli, lyra);
cliToArcApi = createLink(cli, arcApi);
...
$(function() {
  ...
  arrowSourceHidden(dashToLyra.id);
  arrowSourceHidden(dashToArcApi.id);
  arrowSourceHidden(cliToLyra.id);
  arrowSourceHidden(cliToArcApi.id);

  // add popovers
  getJqueryElement(dashboard.id).popover({ title: 'Dashboard', container: '#popover_container'});
});